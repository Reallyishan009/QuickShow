    ...
    try {
      ...
      const response = await axios.get(
        `https://api.themoviedb.org/3/movie/now_playing?api_key=${
          ...
          "6be9d0c3873af0899ce409eb2de46af2"
        }&page=1`
      );
      ...
      <div className="mb-4">
        ...
        <div className="flex flex-wrap gap-2">
          {theater.showtimes.map((time, index) => (
            <button
              key={index}
              className="px-3 py-1 bg-gray-700 hover:bg-primary text-white text-sm rounded transition-colors"
            >
              {time}
            </button>
          ))}
    ...
    <div className="relative min-h-screen pt-32 pb-20 px-6 md:px-16 lg:px-40 xl:px-44">
      ...
      <div className="mb-8">
        <h1 className="text-4xl font-bold text-white mb-4">
          Theaters Near You(this is just for show)
        </h1>
        <p className="text-gray-400 text-lg">
          (Actual booking will happen on Movies page [click through navbar])Find
          the best cinemas and book your favorite movies
        </p>
      </div>
        ...
        {loading ? (
          <div className="grid gap-6">
            {[...Array(3)].map((_, index) => (
              <div
                key={index}
                className="animate-pulse bg-gray-800 rounded-xl p-6"
              >
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {movies.map((movie) => (
            <div key={movie.id} className="group cursor-pointer">
              ...
              <h3 className="text-white font-medium mt-2 text-sm truncate">
                {movie.title}
              </h3>
