...
const AddShows = () => {
  ...
  // Add date and time to selection (avoid duplicates)
  ...
  // Remove specific time from date, remove date if no times left
    ...
    <>
          ...
          {nowPlayingMovies.map((movie) => (
            <div
              ...
              className={`relative max-w-40 cursor-pointer group-hover:not-hover:opacity-40 hover:translate-y-1 transition duration-300 ${
                selectedMovie === movie.id ? 'opacity-100' : 'opacity-80'
              }`}
      ...
      {Object.keys(dateTimeSelection).length > 0 && (
        <div className="mt-4 max-w-xs">
          <p className="font-medium mb-2">Selected Show Dates & Times:</p>
          <div className="flex flex-wrap gap-3">
            {Object.entries(dateTimeSelection).map(([date, times]) =>
              times.map((time) => (
                <div
                  key={`${date}-${time}`}
                  className="bg-primary/20 rounded-md px-3 py-1 flex items-center gap-2"
                >
                  <span>{date} {time}</span>
                  <button
                    type="button"
